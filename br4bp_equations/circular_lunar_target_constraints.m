function [ceq, ceq_grad] = circular_lunar_target_constraints(xf_n, r_mf, stm_n, fixed_orbit_radius_nd, mu_moon_nd, simparams)
...
% Position - all +y from moon
r_target = r_mf + [0; fixed_orbit_radius_nd; 0];

% Velocity - all -x velocity wrt moon velocity
v_scM_M_target = [-sqrt(mu_moon_nd/fixed_orbit_radius_nd); 0; 0];
...
v_sc_sB1_target = v_scM_M_target + v_MB1_B1 + v_B1S_S - w_sB1_x * r_B1S;
...
ceq(1:3) = rf_n - r_target;
ceq(4:6) = vf_n - v_sc_sB1_target;

%% Gradient of position constraint

% drfn_ds0n
drfn_dx0n = stm_n(1:3,:);
drfn_ddtn = vf_n;
drfn_ds0n = [drfn_dx0n, drfn_ddtn];

% drtarget_ds0n
drMfn_dr0n = zeros(3,3);
drMfn_dv0n = zeros(3,3);

drMfn_dth0n = [-1/a4*(1-mu) * sin(theta_emfn); ...
               1/a4* (1-mu) * cos(theta_emfn); ...
               0];

drMfn_ddtn = simparams.theta_em_dot * drMfn_dth0n;

drtarget_ds0n = [drMfn_dr0n, drMfn_dv0n, drMfn_dth0n, drMfn_ddtn];

ceq_grad(1:3,:) = drfn_ds0n - drtarget_ds0n;


%% Gradient of velocity constraint


% dvfn_dsn
dvfn_dx0n = stm_n(4:6,:);
xfn_dot = stateDot(xf_n, simparams);
dvfn_ddtn = xfn_dot(4:6);
dvfn_dsn = [dvfn_dx0n, dvfn_ddtn];



% dvsc_dsn

dvmB1_dsn = w_em_x * [drMfn_dr0n, drMfn_dv0n, drMfn_dth0n, drMfn_ddtn];
...
dvsc_dsn = dvmB1_dsn + dvB1SS_dsn - dwSB1xrsc_dsn;
