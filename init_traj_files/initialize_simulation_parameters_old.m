...
% simparams.P_thresh = 5e7; % m^2
simparams.P_max_r = 5e11; 
% simparams.P_thresh = 5e5; 
% simparams.P_thresh = 7e5; 
...
simparams.sig_vel = 10 / 1e3; % Velocity +/- 10 m/s in all 3 directions
% Double Large
% simparams.sig_pos = 10000 / 1e3; % Position +/- 10 km in all 3 direction
% simparams.sig_vel = 10 / 1e3; % Velocity +/- 10 m/s in all 3 directions
% Huge
...
% simparams.P_initial = diag([50*simparams.sig_pos^2 50*simparams.sig_pos^2 5*simparams.sig_pos^2 20*simparams.sig_vel^2 20*simparams.sig_vel^2 20*simparams.sig_vel^2]);
...
simparams.nom_dvctied = 0; % A flag to force the TCM to occur concurrently with the corresponding nominal impulsive maneuver identified by the following variable
simparams.maneuver_w_corr = 0; % The index of simpar.maneuverSegments where a correction occurs (currently the first nominal maneuver); set to 0 to not tie to a nominal maneuver
%%%%%%%%%%% TO DO: IS THERE A DIFFERENCE OF HAVING A ZERO OR 1 WITH THE
%%%%%%%%%%% ABOVE? (seems like I have a redundant parameter - clean up)
...
simparams.segn_coast_fraction = 0.02; % percent of orbital period to coast in the last segment for initial segment guess
...
simparams.target_final_maneuver = 0;
...
coe_init.inc = 45; % inclination, deg
...
% Target orbit - 
% altitude_targ = 35786; % km, geostationary
...
coe_targ.arglat = 60; % deg, angle between ascending node and satellite position (only for circular inclined orbits)
...
simparams.optoptions.ConstraintTolerance = 1e-8;
