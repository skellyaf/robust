...
% Add coast (in reverse) to get the initial state
...
altitude_targ = 110; % Lunar altitude of 110km
...
simparams.x0(:,simparams.maneuverSegments(1):simparams.maneuverSegments(2)-1) = X;

simparams.x0 = simparams.x0(:);

%% Load det opt

load('leo_llo_det_opt.mat');
simparams.x0 = x_opt;
simparams.n=20;
...
x_new = zeros(simparams.m, length(tcm_time0) + 3);
...
x_new(1:6,2) = simparams.x0(1:6,2); % DV1 position

for i = 1:length(tcm_time0)
    % TCM states
    x_tcm_curr = traj0.x_t( tcm_idx0(i),1:6)';
    x_new(1:6,i+2) = x_tcm_curr;

    % Segment duration
    x_new(7,i+1) = tcm_time0(i) - sum(x_new(7,1:i));
%     x_new(7,i+1) = traj0.t(tcm_idx0(i)) - sum(x_new(7,1:i));

end
t_dv2 = sum(simparams.x0(7,1:end-1));
x_new(7,end-1) = t_dv2 - sum(x_new(7,1:end-2));
...
simparams.tcm_nodes = 3:simparams.n-1;


%% Subdivide any lengthy segments
% Divide segment 3 into 4 segments
[~,x_3_t, t_3] = stateProp(x_new(1:6,3), x_new(7,3), simparams);
x_3_new = subdivide_segment(x_3_t, t_3, 4);

x_new2 = zeros(7,size(x_new,2)+3);
x_new2(:,1:2) = x_new(:,1:2);
x_new2(:,3:6) = x_3_new;
x_new2(:,7:end) = x_new(:,4:end);
...
simparams.tcm_nodes = [simparams.tcm_nodes(1), simparams.tcm_nodes(2:end) + 3];
