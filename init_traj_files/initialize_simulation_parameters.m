...
% simparams.sig_vel = 1 / 1e3 * 3600; % Velocity +/- 1 m/s in all 3 directions
% Large
...
simparams.nom_dvctied = 1; % 1 A flag to force the TCM to occur concurrently with the corresponding nominal impulsive maneuver identified by the following variable
simparams.maneuver_w_corr = 1; % 1 The index of simpar.maneuverSegments where a correction occurs (currently the first nominal maneuver); set to 0 to not tie to a nominal maneuver
...
simparams.seg1_coast_fraction = 0; % percent of orbital period to coast in the first segment for initial segment guess
...
simparams.segn_coast_fraction = 0.05; % percent of orbital period to coast in the last segment for initial segment guess
...
coe_init.arglat = 0; % deg, angle between ascending node and satellite position (only for circular inclined orbits)
