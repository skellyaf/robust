...
simparams.start_P_growth_node = 0; % At which node to allow the covariance to grow via linear dynamics/STM. Another way to think about it: where simparams.P_initial is applied initially
...
X_curr = x_chaser_before_dv1;
T = time_between_dvs;

x_corr_target = x_rdvz;

tDesignFlag = 0;
correction = 1;
while correction

    [x_final, stm, xstm_t] = stateStmProp(X_curr, T, simparams);
...
%     fX_constraint = x_final(constraintEl) - simparams.x_target(constraintEl);
    fX_constraint = x_final(constraintEl) - x_corr_target(constraintEl);
...
[xf_chaser_dv1, x_chaser_postdv1, t_chaser_postdv1] = stateProp(X_return, T, simparams);
