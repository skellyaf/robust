...
simparams.maneuverSegments = [2, 20, simparams.n]; % the segments with an impulsive maneuver at their beginning
...
%%%%%%%%%%%%%%%%%%%%%% playing around with apse constraints
% Make the final segment shorter 
% Potential reasons:
% % Apply an apse constraint right before the flyby delta V
% % Add another flyby distance constraint
% % % simparams.flyby_segment_time = .0001; % shortly before the delta V
% % % 
% % % x_tli_lloFlyby(7,end-1) = x_tli_lloFlyby(7,end) + x_tli_lloFlyby(7,end-1) - simparams.flyby_segment_time;
% % % x_tli_lloFlyby(1:6,end) = stateProp(x_tli_lloFlyby(1:6,end-1), x_tli_lloFlyby(7,end-1), simparams);
% % % x_tli_lloFlyby(7,end) = simparams.flyby_segment_time;
%%%%%%%%%%%%%%%%%%%%%%
%%%%%% IN TANDEM NEED TO ADD A TIME CONSTRAINT ON FLYBY SEG -1
...
coe_flyby.nu = 135 * pi/180; % true anomaly, deg (not used for circular orbits)
% if initial orbit is circular/equatorial, the following are used.
...
[~,x_flyby_dro_t, t_flyby_dro] = stateProp(x_flyby_depart, T_post_flyby, simparams);

% Divide the posty flyby to DRO into segments
num_flyby_to_dro_segments = simparams.maneuverSegments(3) - simparams.maneuverSegments(2);
x_flyby_to_dro = subdivide_segment(x_flyby_dro_t, t_flyby_dro, num_flyby_to_dro_segments);

%%%%%%%%%%%%%%%%%%%%%% playing around with apse constraints
% Make the first segment shorter and the second segment longer
% x_flyby_to_dro(7,2) = x_flyby_to_dro(7,1) + x_flyby_to_dro(7,2) - simparams.flyby_segment_time;
% x_flyby_to_dro(1:6,2) = stateProp(x_flyby_to_dro(1:6,1), simparams.flyby_segment_time, simparams);
% x_flyby_to_dro(7,1) = simparams.flyby_segment_time;
%%%%%%%%%%%%%%%%%%%%%%

% Add to initial traj params
simparams.x0(:,simparams.maneuverSegments(2):simparams.maneuverSegments(3)-1) = x_flyby_to_dro;
...
x0_dro = lagrangeOrbitICs.distant_retrograde.state_nd(:,7);
T_dro = lagrangeOrbitICs.distant_retrograde.T_nd(7);

% Propagate the DRO
% [~,x_dro_t, t_dro] = stateProp(x0_dro, T_dro*.5, simparams);
[~,x_dro_t, t_dro] = stateProp(x0_dro, T_dro*.6, simparams);
...
simparams.x0(7,simparams.maneuverSegments(3)) = T_coast_dro_target;
