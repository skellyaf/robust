...
simparams.P_max_r = 50/3; % km
% simparams.P_max_r = 1200/3; % km
% simparams.P_max_r = 100; % km
...
% TCM execution error
simparams.sig_tcm_error = .1 / 1e3 * 3600; % Velocity 1 sigma = 10 cm/s
% simparams.sig_tcm_error = 1 / 1e3 * 3600; % Velocity 1 sigma = 1 m/s
...
simparams.seg1_coast_fraction = 0.25; % percent of orbital period to coast in the first segment for initial segment guess
...
simparams.segn_coast_fraction = 0; % percent of orbital period to coast in the last segment for initial segment guess
...
coe_init.arglat = 270; % deg, angle between ascending node and satellite position (only for circular inclined orbits)
...
% if initial orbit is circular/equatorial, the following are used.
coe_targ.truelon = nan; % deg, angle between x-axis and satellite (only for circular and equatorial)
coe_targ.arglat = 210; % deg, angle between ascending node and satellite position (only for circular inclined orbits)
