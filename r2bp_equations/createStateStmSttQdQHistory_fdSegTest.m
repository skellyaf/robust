function [traj] = createStateStmSttQdQHistory_fdSegTest(x, traj, modseg, simparams)
...
%         [x_i_final, stm_i(:,:,i), stt_i(:,:,:,i), xstmstt_t_i, t_i] = stateStmSttProp(x_i_initial, delta_t, simparams, eye(6), zeros(6,6,6));
        [x_i_final, traj.stm_i(:,:,i), traj.stt_i(:,:,:,i), traj.Q_i(:,:,i), traj.dQ_i(:,:,:,i), xstmstt_t_i, t_i] = statestmsttQQ2Prop(x_i_initial, delta_t, simparams);

        traj.x_i_f(:,i) = x_i_final;

        if i == 1
            % State history
            traj.x_t = [xstmstt_t_i(:,1:6); traj.x_t(traj.t_s>1,:)];
            ...
            traj.stt_t_i{i} = reshape( xstmstt_t_i(:,43:258)',6,6,6,[] );
            % Mirroring the above with a STM history cell structure
            traj.stm_t_i{i} = stm_t;
            % Time history
            traj.t = [t_i; traj.t(traj.t_s>1)];
            % Segment corresponding to each time
            traj.t_s = [ones([length(t_i), 1]); traj.t_s(traj.t_s>1)];


            % Process noise 
            
            traj.Q_t_i{i} = reshape( xstmstt_t_i(:,259:294)',6,6,[] );
            traj.dQ_t_i{i} = reshape( xstmstt_t_i(:,295:510)',6,6,6,[] );
...
%             t = [t; t_i(2:end) + sum(x(7,1:i-1))];
            traj.t = [traj.t(traj.t_s<modseg); t_i(2:end) + sum(x(7,1:i-1)); traj.t(traj.t_s>modseg)]

            % Corresponding segment
%             t_s = [t_s; i*ones([length(t_i)-1, 1])];
            traj.t_s = [traj.t_s(traj.t_s<modseg); i*ones([length(t_i)-1, 1]); traj.t_s(traj.t_s>i)];

            % State
%             x_t = [x_t; xstmstt_t_i(2:end,1:6)];
            traj.x_t = [traj.x_t(traj.t_s<i); xstmstt_t_i(2:end,1:6); traj.x_t(traj.t_s>i)];

            % Reshape new STM history tensor
            stm_t_i_curr = reshape( xstmstt_t_i(:,7:42)',6,6,[] );
            traj.stm_t_i{i} = stm_t_i_curr;
