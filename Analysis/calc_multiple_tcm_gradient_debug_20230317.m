function [tcm_gradient, tcm_gradient_r, tcm_gradient_v,tcm_gradient_v_test] = calc_multiple_tcm_gradient(x, x_t, x_i_f, stm_i, stt_i, stm_t, stm_t_i, stt_t_i, t, t_s, tcm_time, tcm_idx, tcm_dv_each, P_k_minus, P_k_plus, simparams)
...
for k = 1:num_r_corrections
    ...
    for i = 1:n
        ...
        if k == 1 %%%% if the first correction
            Tlast = 0; dTlastdxi = 0; dTlastddti = 0; PClast_minus = 0; dPClast_minus_dxi = 0; dPClast_minus_ddti = 0;
        else
        ...
        % incrementally verified the above when k=1, i = 1:n
        ...
        % RETURN HERE: THERE IS A DECENTLY LARGE DISCREPANCY WHEN K=2...
        % TRY AND VERIFY SOME OF ITS CONSTITUENTS

        % CHECKING dT...it has a decent percent difference...investigate
        %%%% dTddti may need a fix: TODO: for the second correction,
        %%%% modifications to the first segment duration are affecting the
        %%%% T matrix somehow. Answering myself: it is because the target
        %%%% time is being calculated based on the additional duration
        %%%% added to dt1


        % Assemble TCMv gradient with calculated pieces above
        % CURRENTLY WORKS: BUT - UNANSWERED QUESTIONS. SIMILAR RESULTS
        % REGARDLESS OF WHICH k INDEX IN THE FIRST FOR LOOP. IS ONE MORE
        % ACCURATE?
        % Last one seemed to make the most sense, but there are still
        % slight discrepancies

%         if k == num_r_corrections
% %             dLkdxi(:,:,:,k,i) = L_partial(Wk, Tk, dstmNCkdxi, dTkdxi(:,:,:,k,i));
% %             tcm_gradient_v_test((i-1)*7+1:(i-1)*7+6,1) = calc_dSigVk(Lk, dLkdxi(:,:,:,k,i), P_i_minus(:,:,k), dPCkminusdxi(:,:,:,k,i), simparams);
% %             
% %             dLkddti(:,:,k,i) = L_partial(Wk, Tk, dstmNCkddti, dTkddti(:,:,k,i));
% %             tcm_gradient_v_test((i-1)*7+7,1) = calc_dSigVk(Lk, dLkddti(:,:,k,i), P_i_minus(:,:,k), dPCkminusddti(:,:,k,i), simparams);
% 
% 
% 
%             %% new method
...
% 
%             
% %             sigv = sqrt(trace(Mv*Pn*Mv'));
% %             MPM = tmult(Mv,tmult(dPndxi,Mv'));
% %             for p = 1:6
% %                 dSig(p) = trace( MPM(:,:,p));
% %             end
% % 
% % 
% % 
% %             tcm_gradient_v_test((i-1)*7+1:(i-1)*7+6,1) = 1/2 * sigv^-1 * dSig;
% 
