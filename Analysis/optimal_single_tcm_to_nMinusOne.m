function [t_c_opt, min3sigmaDVC, corrGrad, dvC3sigma, t, t_zerox, stm_t, testVals, x_t, min_3sigma_dvf] = optimal_single_tcm_to_nMinusOne(x, P_initial, mu, varargin)
...
%%%%%%%%%%%%%% ONLY PROPAGATING TO THE END OF SEGMENT N-1 (BECAUSE SEGMENT
%%%%%%%%%%%%%% N IS A COAST IN THE TARGET ORBIT)
for i = 1:n-1
...
for m = 1:length(zxidx)
    ...
    if abs(  sign(slope1) + sign(slope2) + sign(slope3)   ) == 3 || idx1 == 1
        if idx1 == 1
            ...
            [~, stmC1] = stateStmProp(x_i_initial, t_zerox(end) - t_1, mu, eye(6));
        end
