...
maneuverSegments = simpar.maneuverSegments;
...
for i = 1:n
    if ismember(i+1, maneuverSegments)
        idv(:,i) = ( x(4:6,i+1) - x_i_f(4:6,i) ) / deltaViP;
        ...
        deltaV_gradient(i*m-6:i*m-1) = dvgradJ(i*m-6:i*m-1) + dDVndx_minus';

        % Partial of deltaV magnitude at end of current segment
        % with respect to final time for segment
        % ex: partial norm ( v0,2 - vf,1 ) / partial tf,1
        xdot_xif = r2bp_de(1, x_i_final(:,i) );
        vdot_xif = xdot_xif(4:6);
        deltaV_gradient(i*m,1) = dvgradJ(i*m,1) - idv(:,i)' * vdot_xif;
    end
    ...
    if ismember(i, maneuverSegments)
        ...
        deltaV_gradient(i*m-6:i*m-1,1) = dvgradJ(i*m-6:i*m-1,1) + ( idv(:,i-1)' * [zeros(3,3), eye(3)] )';
