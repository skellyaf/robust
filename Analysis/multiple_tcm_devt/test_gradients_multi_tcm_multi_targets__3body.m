...
[tcm_time, tcm_idx, min_tcm_dv, P_i_minus, P_i_plus, event_is_tcm, tcm_dv_each] = opt_multiple_tcm(x, t, stm_t, simparams);
...
k=4; %%%%%% which TCM we're testing from tcm_time / tcm_idx
...
%%%%%%%%%%%%%%%%TODO: potential requirement to fix: the optimal TCM time is
%%%%%%%%%%%%%%%%being recalculated each iteration, which is not the intent.
%%%%%%%%%%%%%%%%May need to modify/write another function to only do the
%%%%%%%%%%%%%%%%calc_covariance part of things with fixed TCM time.
    [tcm_time1dx, tcm_idx1dx, min_tcm_dv1dx, P_i_minus1dx, P_i_plus1dx, event_is_tcm1dx, tcm_dv_each1dx] = opt_multiple_tcm_fdGradientTesting(x1dx, t1dx, stm_t1dx, tcm_time, simparams);
...
%     Pck_minus = P_i_minus(:,:,find(event_times==tcm_time(k)));
    ...
    dPcn_minusdx_fd(:,:,i) = (Pcn_minus1dx - P_i_minus(:,:,6))./dx;
