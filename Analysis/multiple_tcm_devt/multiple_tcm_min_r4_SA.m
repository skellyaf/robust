...
%% SA
%%%%%%%%%%% trying simulated annealing with integers %%%%%%%%%
%%%%%% not sure it is any better - currently takes long. maybe some
%%%%%% settings?

% You only need this for integer optimization. 
...
% initialTemp = [max(xi)-min(xi),max(yi)-min(yi)]; % this will do for the CustomTemperatureFCN
initialTemp = ones(1,4)*3; % this will do for the CustomTemperatureFCN
...
minTemp = 2; % if you're doing integer optimization you want to at least kep the temperature greater than 2 so that your optimization progresses and potentially go uphill

% Solution space quantization. 
quantizationFactor = 1; % check inside the CustomAnnealingFCN for further detaisl. reducing this will improve the solution space quantization. If you set this to 1 it will become integer optimization 



addpath('C:\Users\skell\OneDrive - USU\Documents\code_repos\IntegerOptimization_matlab_SA');
lb = [1,2,3,fourTCMr_idx_best(end)];
ub = fourTCMr_idx_best(end) - [4,3,2,1] + 1;
% Custom options to use integer optimization
options1 = optimoptions('simulannealbnd','FunctionTolerance',1e-8,'AnnealingFcn',{@(x,y)CustomAnnealingFCN(x,y,quantizationFactor)},'TemperatureFcn',{@(x,y) CustomTemperatureFCN(x,y,minTemp,insulationFactor)},...
    'InitialTemperature',initialTemp,'display','iter','ObjectiveLimit',0);
tic
[result,residual,~,out] = simulannealbnd(@(tcm_idx) calc_tcmdv(x, t, stm_t, tcm_idx, simparams),fourTCMr_idx_best,lb,ub,options1); % running SA with integer optimization
toc
%%%%%%%%%%%
%%
...
tic
[fourTCMr_time_best,fourTCMr_idx_best,minDV] = tcm_index_gradient_vector_search(x, t, stm_t, fourTCMr_time_best, fourTCMr_idx_best, 2, simparams);
toc
