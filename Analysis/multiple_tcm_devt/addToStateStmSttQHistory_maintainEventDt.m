function [x_t, stm_t, stm_t_i, stt_t_i, t, t_s, Qbar_t, Q_t_i] = addToStateStmSttQHistory_maintainEventDt(x_t, stm_t, stm_t_i, stt_t_i, t, t_s, Qbar_t, Q_t_i, add_times, simparams)
...
% The original version had a bug, made evident by the following steps:
% 1. A segment duration is extended (seg i-1)
% 2. A TCM occurs in a subsequent segment (i) 
% 3. When the absolute time that a TCM occurs in the subsequent segment is
% calculated, it includes the added dt from the finite different extension
% of segment i-1.
% 4. The current method looks for the absolute TCM time in the absolute
% time vector. This does not account for the dt extension and the TCM is
% performed/applied at a different time in segment i.

% An attempt to rectify: the TCM times need to be adjusted to occur at a reference
% time from the beginning of the segment they are executed in (i).
