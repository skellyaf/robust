function [tcm_feasibleMin_time,tcm_feasibleMin_idx,minTcmDV_meets_constraint, P_tgt_i] = min_dv_tcm_meets_dispersion_constraint_wQ_v3(x, traj, vel_disp_flag, deltaV, P_i, tcm_idx, target_idx, target_time, simparams)
...
% target_time = traj.t(target_idx);

tcm_idx = sort([tcm_idx, target_idx - 1]);
...
event_tgt_i = find(event_idxs == target_idx);
mod_tcm_i = find(tcm_idx == target_idx - 1);




% new_tcm_idx = target_idx - 1;
...
while iter
    ...
    [~, tcm_dv_t(i), ~, P_k_minus] = calc_covariance_wQ_tcmdv_v3(x, traj, tcm_time, 1, deltaV, P_i, Q_k_km1, simparams);

    rP_tcm_time = sqrt(trace(P_k_minus(1:3,1:3,event_tgt_i)));
    
    

    if mode == 1
        if rP_tcm_time > simparams.P_max_r
            if i == 1
...
%                 assert(0);
                ...
                tcm_idx(mod_tcm_i) = tcm_idx(mod_tcm_i) + 1;
                i = i+1;
            end
        else   
        
            tcm_idx(mod_tcm_i) = tcm_idx(mod_tcm_i) - stepSize;
            if tcm_idx(mod_tcm_i) < 1
                tcm_idx(mod_tcm_i) = 2;
