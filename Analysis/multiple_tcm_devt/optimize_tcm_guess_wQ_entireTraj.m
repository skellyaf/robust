...
%     test_logical(range(1):range(1)+25) = true;
    ...
    % Don't test where nominal maneuvers 
...
%             test_logical(maneuverIdx) = false;
            ...
            maneuverIdx = [maneuverIdx, find(maneuver_time==traj.t)];
        end
        test_logical(maneuverIdx) = false;
...
%             [~, plusOneTCMr_totalDV_t(i)] = calc_covariance_wQ_tcmdv(x, traj, tcm_time_test, vel_disp_flag, deltaV, P_i, simparams);    
            ...
            Q_k_km1 = calc_Q_events(traj, x, tcm_time_test, simparams);

            [~, plusOneTCMr_totalDV_t(i)] = calc_covariance_wQ_tcmdv_v3(x, traj, tcm_time_test, vel_disp_flag, deltaV, simparams.P_initial, Q_k_km1, simparams);         
        end
        ...
        [tcm_idx_test, tcm_time_test, minDV] = tcm_index_gradient_vector_search_wQ_entireTraj(x, traj, tcm_idx_test, vel_disp_flag, deltaV, P_i, minDV, simparams);
